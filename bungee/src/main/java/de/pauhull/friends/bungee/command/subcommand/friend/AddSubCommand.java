...
public class AddSubCommand extends SubCommand {
    ...
    public AddSubCommand() {
        ...
        this.setTabPermissions(Permissions.Friends.ADD);
    }
    ...
    public void execute(CommandSender sender, String[] args) {
        ...
        if (!requester.hasPermission(Permissions.Friends.ADD)) {
            requester.sendMessage(TextComponent.fromLegacyText(BungeeFriends.getFriendsPrefix() + friends.getMessages().getNoPermissions()));
            return;
        }

        if (args.length < 2) {
            sender.sendMessage(TextComponent.fromLegacyText(BungeeFriends.getFriendsPrefix() + "§c/friend add <Spieler>"));
            return;
        }
        ...
        if (requester.getName().equalsIgnoreCase(requestedPlayerName)) {
            requester.sendMessage(TextComponent.fromLegacyText(BungeeFriends.getFriendsPrefix() + friends.getMessages().getFriendsNotSelf()));
            return;
        }
                ...
                friends.getUuidFetcher().fetchNameAsync(requestedUUID, name -> {
                    ...
                    friends.getFriendRequestTable().getTime(requestedUUID, requester.getUniqueId(), time -> {
                        if (time != null) {
                            ...
                            requester.sendMessage(TextComponent.fromLegacyText(BungeeFriends.getFriendsPrefix() + String.format(friends.getMessages().getYouAccepted(), name)));

                            ProxiedPlayer requested = ProxyServer.getInstance().getPlayer(requestedUUID);
                            if (requested != null) {
                                requested.sendMessage(TextComponent.fromLegacyText(BungeeFriends.getFriendsPrefix() + String.format(friends.getMessages().getRequestAccepted(), requester.getName())));
                            }
                        } else {
                            friends.getFriendTable().areFriends(requester.getUniqueId(), requestedUUID, alreadyFriends -> {

                                if (alreadyFriends) {
                                    requester.sendMessage(TextComponent.fromLegacyText(BungeeFriends.getFriendsPrefix() + String.format(friends.getMessages().getAlreadyFriend(), name)));
                                } else {
                                    friends.getFriendRequestTable().getTime(requester.getUniqueId(), requestedUUID, timeStamp -> {
                                        if (timeStamp != null) {
                                            requester.sendMessage(TextComponent.fromLegacyText(BungeeFriends.getFriendsPrefix() + String.format(friends.getMessages().getAlreadyRequested(), name)));
                                        } else {
                                            friends.getSettingsTable().isRequests(requestedUUID, receivesRequests -> {

                                                if (receivesRequests) {
                                                    friends.getFriendRequestTable().sendFriendRequest(requester.getUniqueId(), requestedUUID);
                                                    requester.sendMessage(TextComponent.fromLegacyText(BungeeFriends.getFriendsPrefix() + String.format(friends.getMessages().getRequestSent(), name)));

                                                    ProxiedPlayer requested = ProxyServer.getInstance().getPlayer(name);
                                                    if (requested != null) {
                                                        requested.sendMessage(TextComponent.fromLegacyText(BungeeFriends.getFriendsPrefix() +
                                                        ...
                                                        BaseComponent[] message = new ComponentBuilder(BungeeFriends.getFriendsPrefix()).append("§8[").append("§a§lAnnehmen").event(acceptHover).event(acceptClick)
                                                                .append("§8/").append("§c§lAblehnen").event(denyHover).event(denyClick).append("§8]").create();

                                                        requested.sendMessage(message);
                                                    }
                                                } else {
                                                    requester.sendMessage(TextComponent.fromLegacyText(BungeeFriends.getFriendsPrefix() + String.format(friends.getMessages().getReceivesNoRequests(), name)));
                                                }
